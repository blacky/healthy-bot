use sqlx::SqlitePool;
use serenity::all::{Http, ChannelId, GuildId, ChannelType};
...
async fn announce_reminders(pool: &SqlitePool, http: &Http) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let now = chrono::Utc::now();
    let all_reminders: Vec<db::Reminder> = sqlx::query_as::<_, db::Reminder>("SELECT * FROM reminder")
        .fetch_all(pool)
        .await?;

    let reminders: Vec<db::Reminder> = all_reminders.into_iter()
        .filter(|r| r.date_utc() <= now)
        .collect();

    if reminders.is_empty() { return Ok(()); }

    log::info!("Announcing {} expired reminders", reminders.len());

    let channel_id_str = db::get_setting(pool, "main_text_channel").await
...
async fn update_reminders_vc(pool: &SqlitePool, http: &Http) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let category_id_str = db::get_setting(pool, "reminder_category").await
    ...
    let today_start = Amsterdam.with_ymd_and_hms(now_ams.year(), now_ams.month(), now_ams.day(), 0, 0, 0).unwrap().with_timezone(&Utc);
    let today_end = today_start + chrono::Duration::days(1);

    let all_reminders: Vec<db::Reminder> = sqlx::query_as::<_, db::Reminder>(
        "SELECT * FROM reminder ORDER BY date ASC"
    )
    .fetch_all(pool)
    .await?;
    ...
    log::info!("Updating VC reminders (found {} for today)", reminders.len());
    ...
    // Create new ones
    for reminder in reminders {
        let time_str = reminder.date_utc().with_timezone(&Amsterdam).format("%H:%M").to_string();
        let name = format!("{} - {}", time_str, reminder.message);
        log::info!("Creating new VC reminder: {}", name);
        let _ = guild.create_channel(http, serenity::all::CreateChannel::new(name)

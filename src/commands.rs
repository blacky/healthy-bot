...
use crate::db;
use crate::Data;
use chrono::{Utc, NaiveDate, NaiveDateTime, TimeZone, Datelike};
...
/// Manage reminders
#[poise::command(slash_command, prefix_command, rename = "remind", aliases("reminder", "reminders"))]
pub async fn remind_cmd(
    ctx: Context<'_>,
    #[rest] #[description = "Reminder message or action"] input: Option<String>,
) -> Result<(), Error> {
    ...
    if action == "remove" {
        ...
        let reminder: db::Reminder = sqlx::query_as::<_, db::Reminder>("SELECT * FROM reminder WHERE id = ?")
            .bind(id)
            .fetch_one(pool)
            .await?;
        ...
        sqlx::query("DELETE FROM reminder WHERE id = ?").bind(id).execute(pool).await?;
        ctx.say(format!("Successfully removed reminder #{}.", id)).await?;
        return Ok(());
    }

    // Check for listing actions
    let filter = parts.iter().find(|&&p| ["today", "tomorrow", "week", "month"].contains(&p));
    ...
    if input_val.is_empty() || filter.is_some() {
        let mut reminders: Vec<db::Reminder> = if invoked_name == "reminders" {
             sqlx::query_as::<_, db::Reminder>("SELECT * FROM reminder").fetch_all(pool).await?
        } else {
             sqlx::query_as::<_, db::Reminder>("SELECT * FROM reminder WHERE owner_discord_id = ?").bind(&user_id).fetch_all(pool).await?
        };
        ...
        if let Some(&f) = filter {
            reminders.retain(|r| {
                ...
                match f {
                    ...
                    "week" => r_date_ams >= now_ams.date_naive() && r_date_ams <= now_ams.date_naive() + chrono::Duration::days(7),
                    "month" => r_date_ams >= now_ams.date_naive() && r_date_ams <= now_ams.date_naive() + chrono::Duration::days(30),
                    _ => true
                }
            });
        }
        ...
        let mut embed = create_embed("Reminders")
            .footer(CreateEmbedFooter::new(format!("Reminder example: !remind {} f1 time turds", now_ams.format("%d-%m-%Y %H:%M"))));
    ...
    // Add reminder logic
    let is_authorized = {
        let is_owner = ctx.guild().map(|g| g.owner_id == ctx.author().id).unwrap_or(false);
        let has_admin = match ctx.author_member().await {
            Some(member) => member.permissions.map(|p| p.administrator()).unwrap_or(false),
            None => false,
        };
    ...
    if is_authorized {
        let adds: Vec<&str> = input_val.split(|c| c == ';' || c == '\n')
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .collect();
...
pub async fn markov(
    ...
    let cooldown_secs: u64 = {
        ...
        cache.get("markov_cooldown_seconds").and_then(|v| v.parse().ok()).unwrap_or(0)
    };
    ...
    *last_inv = std::time::Instant::now();
    ...
    let target_id = user.map(|u| u.id.to_string()).unwrap_or_else(|| ctx.author().id.to_string());
...
pub async fn settings(
    ...
    key: Option<String>,
    #[rest] #[description = "New value"] value: Option<String>,
) -> Result<(), Error> {
    let pool = &ctx.data().db_pool;
    let user_id = ctx.author().id.to_string();
    db::create_user_if_not_exists(pool, &user_id).await?;
    ...
    if action.as_deref() == Some("set") {
        ...
        if !is_admin { return Err("You are not authorized to change settings.".into()); }
...
pub async fn status(
    ...
    #[rest] #[description = "Status message"] message: String,
) -> Result<(), Error> {
    let pool = &ctx.data().db_pool;
    ...
    if !is_admin { return Err("You are not authorized to change status.".into()); }
    ...
    ctx.say(format!("Status updated to: {} {}", activity_type, message)).await?;
    Ok(())
}

/// User and database management commands
#[poise::command(slash_command, prefix_command, rename = "user", aliases("users", "inthards"))]
pub async fn user_cmd(
    ctx: Context<'_>,
    #[description = "Action (authorize, inthards, sync, latest_message)"] 
    #[autocomplete = "autocomplete_user_action"]
    action: Option<String>,
    #[rest] #[description = "User or ID"] arg: Option<String>,
) -> Result<(), Error> {
    let pool = &ctx.data().db_pool;
    let cmd_name = ctx.command().name.as_str();

    if cmd_name == "inthards" || action.as_deref() == Some("inthards") {
        let top_x = action.as_deref().and_then(|s| s.parse::<i32>().ok()).unwrap_or(5).abs() as usize;
        let users: Vec<db::User> = sqlx::query_as::<_, db::User>("SELECT * FROM users ORDER BY last_message ASC LIMIT ?")
            .bind(top_x as i32)
            .fetch_all(pool)
            .await?;

        let mut embed = create_embed("Inthards Top List")
            .description(format!("Top {} inactive inters", top_x));
    ...
    match action.as_deref() {
        Some("authorize") | Some("sync") | Some("latest_message") => {
            let is_admin = {
                ...
                let db_admin = db::get_user(pool, &ctx.author().id.to_string()).await.map(|u| u.role == "ADMIN").unwrap_or(false);
            ...
            if action.as_deref() == Some("authorize") {
                ...
                ctx.say(format!("<@{}> is {} authorized.", target_id, if target.authorized { "now" } else { "no longer" })).await?;
                ...
                for member in members {
                    if !member.user.bot {
                        ...
                        let _ = sqlx::query("DELETE FROM users WHERE discord_id = ?").bind(member.user.id.to_string()).execute(pool).await;
                    }
                }
                
                ctx.say(format!("Synced {} members with database.", synced_count)).await?;
            } else if action.as_deref() == Some("latest_message") {
                let target_id_str = arg.ok_or("No user id specified")?;
                let target_id: u64 = target_id_str.replace("<@", "").replace(">", "").replace("!", "").parse()?;
                
                let channel_id_str = {
                    let cache = ctx.data().settings_cache.read().await;
                    cache.get("main_text_channel").cloned().ok_or("main_text_channel setting not found")?
                };
                let channel_id: u64 = channel_id_str.parse()?;
                
                let messages = serenity::ChannelId::new(channel_id).messages(ctx.http(), serenity::builder::GetMessages::new().limit(100)).await?;
                
                if let Some(msg) = messages.iter().find(|m| m.author.id.get() == target_id) {
                    ctx.say(format!("<@{}>'s latest message was: {} at <t:{}>", target_id, msg.content_safe(ctx.cache()), msg.timestamp.unix_timestamp())).await?;
                } else {
                    ctx.say(format!("No message found for member <@{}> in the last 100 messages.", target_id)).await?;
                }
            }
        }
        _ => {
            ctx.say("Invalid action. Available: authorize, inthards, sync, latest_message").await?;
        }
    }
...
pub async fn register(ctx: Context<'_>) -> Result<(), Error> {
    ...
    if !is_owner && !is_authorized_user { 
        return Err("Only the server owner or authorized administrators can register commands.".into()); 
    }
...
async fn autocomplete_settings_key(
    _ctx: Context<'_>,
    partial: &str,
) -> Vec<String> {
    let keys = vec![
...
async fn autocomplete_settings_action(
    _ctx: Context<'_>,
    partial: &str,
) -> Vec<String> {
    vec!["get", "set"].into_iter()
...
async fn autocomplete_status(
    ...
    vec!["playing", "watching", "listening", "competing", "custom"].into_iter()
        .filter(move |name| name.to_lowercase().starts_with(&partial.to_lowercase()))
        .map(|name| name.to_string())
        .collect()
}

async fn autocomplete_user_action(
    _ctx: Context<'_>,
    partial: &str,
) -> Vec<String> {
    vec!["authorize", "inthards", "sync", "latest_message"].into_iter()
        .filter(move |name| name.to_lowercase().starts_with(&partial.to_lowercase()))
        .map(|name| name.to_string())
        .collect()
}
